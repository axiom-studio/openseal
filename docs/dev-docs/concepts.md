...
schedules, logical conversation routes, and normalized provider callback
subscriptions.
...
flowchart LR
    ...
    Preview --> Map[Map exact Skills, credentials, destinations, and callback policy]
...
key returns the same resources. Callback requirements carry only event and
consumer intent; the target host supplies provider-local configuration and
creates a fresh ingress route. Exporting the installed Agent produces the same
portable bundle and digest.
