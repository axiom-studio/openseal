...
func (s *PostgresStore) migrateSourceMonitors(ctx context.Context, tx *sql.Tx) error {
	if _, err := tx.ExecContext(ctx, `
		...
		CREATE INDEX IF NOT EXISTS source_observations_monitor_idx ON `+s.table("source_observations")+`(scope_kind,scope_id,project_id,monitor_id,ingested_at DESC);
...
func (s *PostgresStore) IngestSourceObservation(ctx context.Context, observation *SourceObservation, checkpoint *SourceMonitorCheckpoint, expected int64, event *ActivityEvent) (*SourceObservation, *SourceMonitorCheckpoint, *ActivityEvent, bool, error) {
	...
	defer tx.Rollback()
	lockKey := "openseal:source-monitor:" + observation.Scope.Kind + ":" + observation.Scope.ID + ":" + observation.ProjectID + ":" + observation.MonitorID
	if _, err = tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtext($1))`, lockKey); err != nil {
		return nil, nil, nil, false, err
	}
	current, err := s.getPostgresSourceMonitorCheckpointTx(ctx, tx, observation.Scope, observation.ProjectID, observation.MonitorID, true)
	...
	if _, err = tx.ExecContext(ctx, `INSERT INTO `+s.table("source_observations")+`(scope_kind,scope_id,id,project_id,monitor_id,run_id,dedupe_key,ingested_at,payload)VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb)`, observation.Scope.Kind, observation.Scope.ID, observation.ID, observation.ProjectID, observation.MonitorID, observation.RunID, observation.DedupeKey, observation.IngestedAt, string(payload)); err != nil {
		return nil, nil, nil, false, err
	}
	if err = s.upsertPostgresSourceMonitorCheckpointTx(ctx, tx, next, expected); err != nil {
		return nil, nil, nil, false, err
...
func (s *PostgresStore) AdvanceSourceMonitorCheckpoint(ctx context.Context, checkpoint *SourceMonitorCheckpoint, expected int64, event *ActivityEvent) (*SourceMonitorCheckpoint, *ActivityEvent, bool, error) {
	...
	defer tx.Rollback()
	lockKey := "openseal:source-monitor:" + checkpoint.Scope.Kind + ":" + checkpoint.Scope.ID + ":" + checkpoint.ProjectID + ":" + checkpoint.MonitorID
	if _, err = tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtext($1))`, lockKey); err != nil {
		return nil, nil, false, err
	}
	current, err := s.getPostgresSourceMonitorCheckpointTx(ctx, tx, checkpoint.Scope, checkpoint.ProjectID, checkpoint.MonitorID, true)
	if err != nil && !errors.Is(err, ErrSourceObservationNotFound) {
		return nil, nil, false, err
	}
...
func (s *PostgresStore) upsertPostgresSourceMonitorCheckpointTx(ctx context.Context, tx *sql.Tx, checkpoint *SourceMonitorCheckpoint, expected int64) error {
	...
	if expected == 0 {
		_, err = tx.ExecContext(ctx, `INSERT INTO `+s.table("source_monitor_checkpoints")+`(scope_kind,scope_id,project_id,monitor_id,revision,updated_at,payload)VALUES($1,$2,$3,$4,$5,$6,$7::jsonb)`, checkpoint.Scope.Kind, checkpoint.Scope.ID, checkpoint.ProjectID, checkpoint.MonitorID, checkpoint.Revision, checkpoint.UpdatedAt, string(payload))
		return err
	}
	result, err := tx.ExecContext(ctx, `UPDATE `+s.table("source_monitor_checkpoints")+` SET revision=$1,updated_at=$2,payload=$3::jsonb WHERE scope_kind=$4 AND scope_id=$5 AND project_id=$6 AND monitor_id=$7 AND revision=$8`, checkpoint.Revision, checkpoint.UpdatedAt, string(payload), checkpoint.Scope.Kind, checkpoint.Scope.ID, checkpoint.ProjectID, checkpoint.MonitorID, expected)
	if err != nil {
		return err
	}
...
func (s *PostgresStore) getPostgresSourceMonitorCheckpointTx(ctx context.Context, tx *sql.Tx, scope Scope, projectID, monitorID string, lock bool) (*SourceMonitorCheckpoint, error) {
	query := `SELECT payload FROM ` + s.table("source_monitor_checkpoints") + ` WHERE scope_kind=$1 AND scope_id=$2 AND project_id=$3 AND monitor_id=$4`
	if lock {
		query += ` FOR UPDATE`
	}
	var payload []byte
	err := tx.QueryRowContext(ctx, query, scope.Kind, scope.ID, projectID, monitorID).Scan(&payload)
...
func (s *PostgresStore) getPostgresSourceObservationByDedupeTx(ctx context.Context, tx *sql.Tx, observation *SourceObservation) (*SourceObservation, error) {
	var payload []byte
	err := tx.QueryRowContext(ctx, `SELECT payload FROM `+s.table("source_observations")+` WHERE scope_kind=$1 AND scope_id=$2 AND project_id=$3 AND monitor_id=$4 AND dedupe_key=$5`, observation.Scope.Kind, observation.Scope.ID, observation.ProjectID, observation.MonitorID, observation.DedupeKey).Scan(&payload)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrSourceObservationNotFound
	}
	if err != nil {
...
func (s *PostgresStore) ListSourceObservations(ctx context.Context, filter SourceObservationFilter) ([]*SourceObservation, error) {
	...
	var retainedAt interface{}
	if !filter.RetainedAt.IsZero() {
		retainedAt = filter.RetainedAt.UTC()
	}
	rows, err := s.db.QueryContext(ctx, `SELECT payload FROM `+s.table("source_observations")+` WHERE scope_kind=$1 AND scope_id=$2 AND ($3='' OR project_id=$3) AND ($4='' OR monitor_id=$4) AND ($5='' OR run_id=$5) AND ($6='' OR payload->>'actionCallId'=$6) AND ($7::timestamptz IS NULL OR payload->>'retentionExpiresAt' IS NULL OR (payload->>'retentionExpiresAt')::timestamptz>$7) ORDER BY ingested_at DESC,id DESC LIMIT $8 OFFSET $9`, filter.Scope.Kind, filter.Scope.ID, filter.ProjectID, filter.MonitorID, filter.RunID, filter.ActionCallID, retainedAt, limit, filter.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *PostgresStore) GetSourceMonitorCheckpoint(ctx context.Context, scope Scope, projectID, monitorID string) (*SourceMonitorCheckpoint, error) {
	var payload []byte
	err := s.db.QueryRowContext(ctx, `SELECT payload FROM `+s.table("source_monitor_checkpoints")+` WHERE scope_kind=$1 AND scope_id=$2 AND project_id=$3 AND monitor_id=$4`, scope.Kind, scope.ID, strings.TrimSpace(projectID), strings.TrimSpace(monitorID)).Scan(&payload)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrSourceObservationNotFound
	}
	if err != nil {
