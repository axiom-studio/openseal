...
const sensitiveAuthoringPromptMigrationVersion int64 = 29
...
func sanitizeAgentDefinitionPayload(payload string) (string, string, bool, error) {
...
func (s *PostgresStore) migrateSensitiveAuthoringPrompts(ctx context.Context, tx *sql.Tx) error {
	...
	for _, value := range definitions {
		payload, digest, changed, err := sanitizeAgentDefinitionPayload(value.payload)
		if err != nil {
			return err
		}
		if changed {
			if _, err := tx.ExecContext(ctx, `UPDATE `+s.table("agent_definitions")+` SET digest=$1,payload=$2::jsonb WHERE id=$3 AND version=$4`, digest, payload, value.id, value.version); err != nil {
				return err
			}
		}
	}
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO `+s.table("schema_migrations")+` (version,name) VALUES ($1,'redact sensitive authoring prompts') ON CONFLICT(version) DO NOTHING`, sensitiveAuthoringPromptMigrationVersion)
	return err
}
