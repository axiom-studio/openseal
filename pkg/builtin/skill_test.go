...
func TestAttachmentSkillIsReadOnlyAndReferenceOnly(t *testing.T) {
	...
	if d.ID != SkillID || len(d.Actions) != 3 || string(a.Risk) != "read" || string(a.SideEffect) != "read" {
	...
	if !d.Requirements.AlwaysAvailable || len(d.Prompt.AllowedTools) != 3 || d.Prompt.AllowedTools[0] != ReadAttachment {
