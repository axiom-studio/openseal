...
func TestHostedRunbookBudgetRaisesBrowserCatalogToExecutableEnvelope(t *testing.T) {
	...
	if budget.MaxAttempts != 7 || budget.MaxTurns != 4 || budget.MaxInputTokens != wantInput || budget.MaxOutputTokens != wantOutput ||
...
func TestHostedRunbookBudgetRejectsTriggerThatCannotFundDelegatedExecution(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if budget.MaxAttempts != 9 || budget.MaxTurns < 6 || budget.MaxInputTokens < 100000 || budget.MaxOutputTokens < capability.HostedMinimumChildOutputTokens || budget.MaxTotalTokens < 116384 {
