...
func authoringModelSystemPrompt() string {
	return authoringSystemPrompt + `
authoringForm is a trusted typed form supplied by the compiler. Treat each enabled field as a semantic input with the stated help, options, and conditions. Fill only the field path; never author compilerOutput fields because OpenSeal derives those immutable runtime fields deterministically after generation.` + authoringSourceIdentityPrompt + authoringSkillOptionActionsPrompt + "\n" + runbook.AuthoringSchemaProjection()
}
...
const strictAuthoringTransportPrompt = `TRANSPORT CONTRACT: Return the authoring response in the strict transport envelope selected by the provider request. candidateJson is the complete candidate JSON object encoded as a JSON string. authoringJson is the complete typed authoring form submission encoded as a JSON string. unresolvedQuestionsJson is the complete unresolvedQuestions JSON array encoded as a JSON string. commitments and assumptions remain structured values. Use null for an unstated nullable commitment and [] for an empty list. Do not omit transport fields.`
...
func strictAuthoringResponseFormat() map[string]interface{} {
	...
	schema := map[string]interface{}{
		...
		"properties": map[string]interface{}{
			...
			"authoringJson":           map[string]interface{}{"type": "string"},
		...
		"required": []string{"candidateJson", "authoringJson", "commitments", "assumptions", "unresolvedQuestionsJson"},
	}
...
func decodeStrictAuthoringTransport(content []byte) ([]byte, error) {
	var envelope struct {
		...
		AuthoringJSON           string            `json:"authoringJson"`
	...
	authoring := json.RawMessage(strings.TrimSpace(envelope.AuthoringJSON))
	...
	if !json.Valid(authoring) || len(authoring) == 0 || authoring[0] != '{' {
		return nil, errors.New("strict authoring transport authoringJson must contain one JSON object")
	}
	...
	return json.Marshal(struct {
		...
		Candidate: candidate, Authoring: authoring, Commitments: envelope.Commitments,
