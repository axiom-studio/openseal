...
TeamDefinition required fields: id, version, displayName, purpose, roles:[{id,displayName,purpose,minimumMembers,maximumMembers,requiredSkillIds,requiredDefinitionIds,skillGrants:[{skillId,skillVersion,allowedActions,enablePrompt,maximumRisk}],channelParticipation}], coordination:{mode}; approvals:{maximumRisk}. A role Skill grant uses the exact catalog Skill id and declared catalog version; never emit catalogId or runtimeIdentity because authorized placement resolves source-qualified runtime identity after review. coordination mode is dynamic, peer, or leader_facilitated. channelParticipation is active, observe_only, or disabled. Every prompt-created Team must have at least one active role so its members can participate in the Team channel; use observe_only only for roles that should read shared context without speaking, and never make every role observe_only or disabled. Role minimumMembers and maximumMembers, coordination maximumSpeakersPerRound, and delegation maximumDepth/maximumConcurrent are JSON integers, never strings. Optional operatingPrinciples, coordination.maximumSpeakersPerRound/quietByDefault/requireRoleRelevance/suppressDuplicateContent, delegation:{maximumDepth,maximumConcurrent,allowPeerDelegation,requireAcceptance,requireCompletionReview}, sharedContext:{retention,maximumBytes,allowMemberRead,allowMemberWrite}, objectiveTemplates, evaluations, amendments, provenance.
