...
func deriveRuntimeCompositionRequirements(prompt string, catalog CapabilityCatalog) *RuntimeCompositionRequirements {
	if meetingVoiceUsesAgentConversation(prompt, catalog) {
		return nil
	}
...
// A meeting voice session is started by an Agent action and sends observations
...
func meetingVoiceUsesAgentConversation(prompt string, catalog CapabilityCatalog) bool {
	skill, newSkill := catalog.Skills["openseal.meeting.voice"]
	available := newSkill
	if !available {
		skill, available = catalog.Skills["openseal.meet.voice"]
	}
	if !available || !containsExactString(skill.Actions, "meet-start") {
		return false
	}
	lower := strings.ToLower(prompt)
	if !strings.Contains(lower, "google meet") &&
		!(newSkill &&
			(strings.Contains(lower, "zoom") || strings.Contains(lower, "teams meeting") ||
				strings.Contains(lower, "microsoft teams"))) {
		return false
	}
	tokens := compositionTokens(prompt)
	return !containsAnyToken(tokens, "slack", "discord", "telegram", "whatsapp", "sms", "webchat") &&
		!strings.Contains(lower, "web chat") &&
		!strings.Contains(lower, "teams chat") && !strings.Contains(lower, "teams channel")
}
