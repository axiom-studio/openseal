...
func TestKernelHTTPClientEncodesSourceMonitorRoutes(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		case "/api/v1/projects/project%2Fone/source-monitors/reddit%2Fnew/checkpoint":
			...
			_ = json.NewEncoder(w).Encode(runtime.SourceMonitorCheckpoint{ProjectID: "project/one", MonitorID: "reddit/new", Revision: 3})
		case "/api/v1/projects/project%2Fone/source-monitors/reddit%2Fnew/observations":
	...
	checkpoint, err := client.GetSourceMonitorCheckpoint(context.Background(), scope, "project/one", "reddit/new")
	if err != nil || checkpoint.Revision != 3 {
		t.Fatalf("checkpoint=%#v err=%v", checkpoint, err)
	}
	observations, err := client.ListSourceObservations(context.Background(), runtime.SourceObservationFilter{Scope: scope, ProjectID: "project/one", MonitorID: "reddit/new", Limit: 20, Offset: 4})
