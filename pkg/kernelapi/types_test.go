...
func TestWorkforceAuthoringVersionDeclaresDigestBoundActivationContract(t *testing.T) {
	capability := WorkforceAuthoringCapability(WorkforceAuthoringCapabilityFeatures{ChangeSets: true})
	if capability.Version != "8" || !capability.Supports(OperationCompile) || !capability.Supports(OperationPropose) || capability.Supports(OperationRefine) || capability.Supports(OperationPatch) {
