# OpenSeal desktop implementation notes

Feature behavior, implementation boundaries, and milestone verification records.
...
Posting records conversation and can trigger model-backed replies when Team replies
is explicitly enabled. It does not resolve agent requests, approve actions, or
submit the separate explicit Team work workflow.
...
samples, not full-page coverage. Other-participant receipt UI, reference kinds beyond work/artifacts/agent requests/approvals, marketplace, and platform delivery remain
...
## Automatic Team replies

In Channel settings, **Enable team replies** opts this channel into automatic
model-backed contributions for new messages. Enabling skips existing history;
disabling and re-enabling establishes a new boundary. Up to eight eligible team
members can review a message and propose a relevant reply or silence. Inactive
agents and observe-only/disabled roles do not reply. The workspace provider is
used and calls may incur charges. Explicit Team work remains a separate workflow;
the reply adapter has no external-action executor or general tool access.

The saved on/off setting uses the existing revision-checked update and persistent
recovery journal. Storage failure prevents a send; uncertain/conflicting delivery
requires **Check saved channel**, without automatic replay. Enabling requires an
active channel, a ready provider, and canonical team eligibility. Disabling remains
available without a provider. Archive turns replies off; restore keeps them off.
Queued proposals recheck channel revision before provider calls. Already-started
requests may still incur charges, and turning replies off does not undo committed
work or cancel explicit team tasks.

The desktop daemon registers conversation workers when the provider is ready.
Host limits are eight participants, two concurrent proposals, 30 recent messages,
and 16,000 input/2,048 output tokens per participant. Each conversation run is
bounded to three turns, four attempts, 432,000 total tokens, 49,152 output tokens,
and 180,000 ms. Authored quiet/relevance/duplicate-suppression rules are respected;
the host maximum of three speakers can be narrowed by the team definition.
Committed rounds retain their policy for recovery. Desktop clients cannot submit
manual agent proposals; worker-produced replies retain canonical authority.

All 106 desktop browser tests, full Go suites, race checks, and Linux native smoke
passed. Verification used synthetic providers and does not establish live model
quality or other-platform support. The bounded review in
`.impeccable/review/channel-participation-finish.md` identified stale design
behavior as its sole required correction; this documentation refresh addresses it.
Its desktop/mobile/native captures are valid scrolled viewport samples, not
...
arrays now have separate bounded support described below. Other-participant receipt UI, marketplace,
...
other-participant receipt UI, marketplace, and platform delivery remain
...
valid intentionally scrolled samples. Other reference kinds, other-participant receipt UI, marketplace, platform delivery, live quality, and
...
alongside remaining reference kinds, marketplace, and
...
toolbars use existing field tokens. Other-participant receipt UI,
...
are not certified. Marketplace, platform delivery, and broader app
