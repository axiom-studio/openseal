...
def main():
    ...
    with model_fixture() as fixture, tempfile.TemporaryDirectory(prefix='openseal-native-smoke-') as directory:
        ...
        try:
            ...
            execute("const remove = Storage.prototype.removeItem; Storage.prototype.removeItem = function(key) { if(key==='openseal.home-submission') throw new Error('Synthetic cleanup interruption'); return remove.call(this,key); }; return true")
            ...
            until("return document.querySelector('[aria-label=\"Saved request recovery\"]')?.innerText.includes('Saved in the workspace, but local recovery could not be cleared')")
            saved_request_key = execute("return JSON.parse(localStorage.getItem('openseal.home-submission')).key")
            saved_proposal_id = execute("return localStorage.getItem('openseal.proposal')")
            assert execute("return document.querySelector('#prompt').readOnly")
            call('/session/' + session + '/window/rect', {'width': 1280, 'height': 840})
            execute("document.querySelector('.composer').scrollIntoView({block:'center'}); return true")
            (output.parent / 'home-recovery-native-linux.png').write_bytes(base64.b64decode(call('/session/' + session + '/screenshot')))
            execute("window.__recoveryReload=true; location.reload(); return true")
            until("return !window.__recoveryReload && document.readyState==='complete' && [...document.querySelectorAll('button')].some(b=>b.textContent.trim()==='Retry saved request' && !b.disabled)")
            assert execute("return JSON.parse(localStorage.getItem('openseal.home-submission')).key") == saved_request_key
            execute("[...document.querySelectorAll('button')].find(b=>b.textContent.trim()==='Retry saved request').click(); return true")
            until("return localStorage.getItem('openseal.home-submission')===null")
            assert execute("return localStorage.getItem('openseal.proposal')") == saved_proposal_id
            ...
            execute("[...document.querySelectorAll('button')].find(b=>b.textContent.trim()==='Show reply activity').click(); return true")
            until("return document.querySelector('[aria-label=\"Reply activity\"]')?.innerText.includes('Completed — 1 message posted')")
            execute("document.querySelector('[aria-label=\"Reply activity\"]').scrollIntoView({block:'center'}); return true")
            (output.parent / 'channel-reply-activity-native-linux.png').write_bytes(base64.b64decode(call('/session/' + session + '/screenshot')))
            execute("[...document.querySelectorAll('button')].find(b=>b.textContent.trim()==='Hide reply activity').click(); return true")
